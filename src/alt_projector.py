...
def latent_interpolation(
    ...
    # Linearly interpolate between latent vectors 1 and 2.
    interpolation = torch.linspace(latent_vector_1, latent_vector_2, interpolation_steps)
    
    # Generate images from interpolated latent vectors.
    generated_images = G.synthesis(interpolation, noise_mode='const')
    generated_images = (generated_images + 1) * (255/2)
    generated_images = generated_images.permute(0, 2, 3, 1).clamp(0, 255).to(torch.uint8).cpu().numpy()
    
    video = imageio.get_writer(f'{outdir}/proj.mp4', mode='I', fps=10, codec='libx264', bitrate='16M')
    print (f'Saving interpolation video "{outdir}/proj.mp4"')
    for step in generated_images:
        video.append_data(np.concatenate([target_uint8, step], axis=1))
    video.close()
