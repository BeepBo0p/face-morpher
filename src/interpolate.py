...
def inverse_distance_interpolation(img1: np.ndarray, img2: np.ndarray, features1: np.ndarray, features2: np.ndarray, n: int, q: float, out_dir: str) -> tuple[str, list[str]]:
    ...
    for t in timesteps:
        ...
        print(
            f'Interpolated features: {[round(f, 2) for f in interpolated_features[0]]}', end=' | ')

        # Store the deltas in positive and negative t-direction together
        interpolated_pos_delta = np.concatenate(
            (-1 * interpolated_pos_delta, reverse_interpolated_pos_delta), axis=1)
        print(
            f'Interpolated delta: {[round(d, 2) for d in interpolated_pos_delta[0]]}')
        ...
        for x in range(w):
            for y in range(h):
                ...
                interpolated_delta_field[x][y] = inverse_distance_weighting(np.array(
                    [x, y], dtype=float), interpolated_features.astype(float), interpolated_pos_delta, q)
        ...
        # Prepend a 0 to the image number if it is less than 10 to ensure correct ordering
        if image_nr < 10:
            cv.imwrite(os.path.join(os.getcwd(
            ), f'output/interpolation/interpolated_image_0{int(image_nr)}.png'), interpolated_image)
            image_path_list.append(f'interpolated_image_0{int(image_nr)}.png')
        else:
            cv.imwrite(os.path.join(os.getcwd(
            ), f'output/interpolation/interpolated_image_{int(image_nr)}.png'), interpolated_image)
...
def inverse_distance_weighting(point: np.array, interpolants: np.ndarray, interpolants_value: np.ndarray, q: float) -> np.ndarray:
    ...
    weighted_values = [weights[i] * interpolants_value[i]
                       for i in range(interpolants.shape[0])]
...
def bilinear_sampling(img: np.ndarray, x: float, y: float) -> np.ndarray:
    ...
    nearest_pixels = [top_left_px, top_right_px,
...
def test_inverse_distance_interpolation():
    ...
    interpolated_image = inverse_distance_interpolation(
        img1, img2, features1, features2, n, q=4)
...
def test_inverse_distance_weighting():
    ...
    for x in range(w):
        for y in range(h):
            ...
            img[x][y] = inverse_distance_weighting(np.array(
                [x, y], dtype=float), interpolants.astype(float), interpolants_value.astype(float), 4)
...
def test_bilinear_sampling():
    ...
    for i in outer_corners:
        print(
            f'Sampled value at {i[0]}, {i[1]}: {bilinear_sampling(sample_image.data, i[0], i[1])}')
    ...
    for h in range(resolution + 1):
        ...
        for w in range(resolution + 1):
            ...
            sampled_image[w][h] = bilinear_sampling(
                sample_image.data, w_t, h_t)
